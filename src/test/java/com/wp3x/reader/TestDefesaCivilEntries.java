...
public class TestDefesaCivilEntries {
	...
	final URL resource = this.getClass().getClassLoader()
			.getResource("defesa-civil-list.htm");
	final URL link1 = this.getClass().getClassLoader()
			.getResource("defesa-civil-n1.htm");
	final URL link2 = this.getClass().getClassLoader()
			.getResource("defesa-civil-n2.htm");
	...
	public void testGetLinks() throws Exception {
		...
		assertEquals(
				"Frente fria traz chuva forte e frio para SC e Fim de semana mar agitado na costa catarinense ",
				firstEntry.getTitle());
		assertEquals("12 | ABR | 2013", firstEntry.getDate());
		assertEquals(
				"/noticia/131-Frente+fria+traz+chuva+forte+e+frio+para+SC+e+Fim+de+semana+mar+agitado+na+costa+catarinense+.html",
				firstEntry.getUrl());
		assertEquals(
				"http://defesacivil.joinville.sc.gov.br/noticia/131-Frente+fria+traz+chuva+forte+e+frio+para+SC+e+Fim+de+semana+mar+agitado+na+costa+catarinense+.html",
				firstEntry.getFormattedURL());
		assertEquals(1365735600000L, firstEntry.getDateAsLong().longValue());
